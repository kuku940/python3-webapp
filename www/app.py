...
# ---------------拦截器middlewares设置-------------
async def logger_factory(app, handler):  # 在正式处理之前打印日志
...
async def response_factory(app, handler):
    ...
    响应处理
        请求在服务端收到后方法调用顺序是：
            logger_factory -> response_factory -> RequestHandler().__call__ -> get/post -> handler
        结果处理情况：
            1、由handler构造出要返回的具体对象
            2、然后在这个返回的对象上加上'__method__'和'__route__'属性，以标识别这个对象并使接下来的程序容易处理
            3、RequestHandler目的就是从URL函数中分析其需要接收的参数，从request中获取必要的参数，
                调用URL函数,然后把结果返回给response_factory
            4、response_factory在拿到经过处理后的对象，经过一系列对象类型和格式的判断，
                构造出正确web.Response对象，以正确的方式返回给客户端
        在这个过程中，我们只用关心我们的handler的处理就好了，其他的都走统一的通道，
        如果需要差异化处理，就在通道中选择适合的地方添加处理代码。
...
async def init(loop):
    ...
    # await orm.create_pool(loop=loop, host='127.0.0.1', port=3306, user='pywebapp', password='123456', db='pywebapp')
