...
async def create_pool(loop, **kw):  # 引入关键字后不用显示import asyncio了
    '''
    创建数据库连接池
    :param loop:
    :param kw:
    ...
    print("create database connection pool success...")
...
# 注意：之所以放在Model类里面作为静态函数处理是为了更好的功能内聚，便于维护，这点与廖老师的处理方式不同，请注
...
async def select(sql, args, size=None):
    ...
    :param sql: sql语句
    :param args: 参数
    :param size: 取记录数,fetchmany获取最多指定数量的记录，否则，fetchall()获取所有记录
    :return: 结果集
    """
...
async def execute(sql, args, autocommit=True):
    """
    execute方法只返回结果数，不返回结果集,用于insert,update,delete这些SQL语句
...
class ModelMetaclass(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        for k in mappings.keys():  # 清除mappings，防止实例属性覆盖类的同名属性，造成运行时错误
            # attrs中对应的属性则需要删除。
            # 作者指的是attrs的属性和mappings中的属性发生冲突，
            # 具体原因可能需要自己实际体验下这个错误才知道
            attrs.pop(k)
...
class Model(dict, metaclass=ModelMetaclass):
    ...
    def getValue(self, key):
        ...
        # 获取实例的key，None是默认值，
        # getattr方法使用可以参考http://kaimingwan.com/post/python/pythonzhong-de-nei-zhi-han-shu-getattr-yu-fan-she
    ...
    @classmethod  # 用于不创建对象,直接可以调用方法,类似静态方法
    ...
    async def findNumber(cls, selectField, where=None, args=None):
        ...
        if len(rs) == 0:  # 结果集为0的情况
            return None
        return rs[0]['_num_']  # 有结果则rs这个list中词典元素_num_的value值
